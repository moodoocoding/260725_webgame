...
if ($action === 'list') {
    ...
    // [정렬 로직] 오름차순 (시간이 작을수록 1등)
    usort($filtered, function($a, $b) {
        if ($a['score'] == $b['score']) {
            return 0;
        }
        // $a가 $b보다 작으면 -1 (앞으로 보냄) -> 오름차순
...
if ($action === 'submit') {
    ...
    if (flock($fp, LOCK_EX)) {
        ...
        $myRank = 1;
        foreach($data as $d) {
            // 닉네임, 점수(float), 타임스탬프 일치 확인
            // float 비교는 엄격하게 하면 가끔 오차가 있을 수 있으나, 
            // JSON 인코딩/디코딩 과정에서 값이 고정되므로 이 방식은 유효함
