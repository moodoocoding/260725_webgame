...
if ($action === 'submit') {
    ...
    $check_key = (int)($_POST['check_key'] ?? 0); // 클라이언트에서 보낸 암호 코드

    // [보안 검증 로직 시작] ----------------------------------
    // 자바스크립트의 _salt(521)와 _base(999) 값과 일치해야 함
    $salt = 521;
    $base = 999;
    
    // 서버에서 직접 계산
    $real_code = ($score * $salt) + $base;

    // 계산 결과가 다르면 조작된 요청으로 간주하고 차단
    if ($check_key !== $real_code) {
    ...
    if (flock($fp, LOCK_EX)) {
        ...
        // 만약 300등 밖으로 밀려나서 못 찾았다면 순위 없음 처리
        if (!$found) $myRank = 301;
